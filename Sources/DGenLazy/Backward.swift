...
extension LazyGraph {
    ...
    func setupGradients(loss: NodeID, frameCount: Int) -> (tensorCells: [NodeID: CellID], signalCells: [NodeID: CellID]) {
    ...
    func readGradients(context: ExecutionContext) -> (tensorGrads: [NodeID: [Float]], signalGrads: [NodeID: Float]) {
        let registry = parameterRegistry
        ...
        for tensor in registry.tensors {
            ...
            let physicalCell = context.compilationResult.cellAllocations.cellMappings[gradCell] ?? gradCell
...
extension Tensor {
    ...
    /// ```
    public func backward(frameCount: Int = DGenConfig.defaultFrameCount) throws {
        ...
        // Populate .grad properties
        for tensor in graph.parameterRegistry.tensors {
            if let grads = tensorGrads[tensor.nodeId] {
                // Create a gradient tensor with the computed values
                tensor.grad = Tensor(grads, requiresGrad: false)
            }
        }

        for signal in graph.parameterRegistry.signals {
            if let gradValue = signalGrads[signal.nodeId] {
                // Create a gradient signal with the computed value
                signal.grad = Signal(gradValue, requiresGrad: false)
            }
        }
    }
}
...
extension Signal {
    ...
    public func backward(frames: Int = DGenConfig.defaultFrameCount) throws {
        // Add output node for the loss
        let _ = graph.node(.output(0), [nodeId])
