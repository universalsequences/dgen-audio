...
public class Signal: LazyValue {
    ...
    public init(_ value: Float, requiresGrad: Bool = false) {
        ...
        if requiresGrad {
            ...
            cellId = cell
            // Store initial value - will be injected at runtime
            // TODO: Need mechanism to inject initial values
        } else {
    ...
    internal init(nodeId: NodeID, graph: LazyGraph, requiresGrad: Bool = false, cellId: CellID? = nil) {
        self.nodeId = nodeId
        self.graph = graph
        self.requiresGrad = requiresGrad
        self.cellId = cellId
    ...
    public static func param(_ value: Float) -> Signal {
        return Signal(value, requiresGrad: true)
    }
    ...
    public static func accum(
        ...
        let needsGrad = increment.requiresGrad ||
                       (reset?.requiresGrad ?? false) ||
                       (min?.requiresGrad ?? false) ||
