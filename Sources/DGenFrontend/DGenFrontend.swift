...
public class GraphBuilder: Graph {
  ...
  /// Biquad filter
  public func biquad(_ input: Node, _ cutoff: Node, _ resonance: Node, _ gain: Node, _ mode: Node)
    -> Node
  {
  ...
  public func compressor(
    ...
    return Node(
      id: compressor(
        input.id, ratio.id, threshold.id, knee.id, attack.id, release.id, isSideChain.id,
        sidechainIn.id),
      graph: self
    )
  }
