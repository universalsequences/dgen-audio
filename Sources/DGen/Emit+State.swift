...
extension LazyOp {
  func emitStateOp(b: IRBuilder, ctx: IRContext, g: Graph, node: Node, inputs: [Lazy], nodeId: NodeID) throws {
    ...
    case .gemmReduceToCell(let M, let N, let K, let transA, let transB, let targetCell):
      guard node.inputs.count == 2,
        ...
        throw DGenError.tensorError(
          op: "gemmReduceToCell", reason: "could not resolve input tensors")
      }

      guard M % 8 == 0, N % 8 == 0, K % 8 == 0 else {
        throw DGenError.tensorError(
          op: "gemmReduceToCell", reason: "requires M/N/K multiples of 8")
      }
      ...
      let leftFrameInfo = ctx.g.frameAwareCells[leftCell]
      let rightFrameInfo = ctx.g.frameAwareCells[rightCell]
      let leftFrameAware = leftFrameInfo != nil
      let rightFrameAware = rightFrameInfo != nil
      let frameLoopCount = leftFrameInfo?.1 ?? rightFrameInfo?.1 ?? 1

      if let leftFrames = leftFrameInfo?.1, let rightFrames = rightFrameInfo?.1, leftFrames != rightFrames {
        throw DGenError.tensorError(
          op: "gemmReduceToCell",
          reason: "frame-aware input frame counts mismatch (\(leftFrames) vs \(rightFrames))")
      }
      ...
      b.loop(frameLoopCount) { frameIdx in
        let frameIdxInt = b.cast(frameIdx, to: .int)
        let leftFrameBase =
          leftFrameAware ? frameIdxInt * b.intConstant(M * K) : b.intConstant(0)
        let rightFrameBase =
          rightFrameAware ? frameIdxInt * b.intConstant(K * N) : b.intConstant(0)
      ...
      let outOffset = tileRow * b.intConstant(8 * N) + tileCol * b.intConstant(8)
      b.simdgroupStore(acc, cellId: targetCell, offset: outOffset, stride: N)
