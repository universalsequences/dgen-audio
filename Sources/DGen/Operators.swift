...
func u_spectralLossBackwardPass1(
    ...
    return { b in
        ...
        // For each sample position - accumulate contributions from ALL bins before writing
        b.loop(windowSize) { n in
            let nFloat = b.cast(n, to: .float)
            let totalContrib1 = b.float(0.0)
            let totalContrib2 = b.float(0.0)

            // Sum contributions across ALL frequency bins
            b.loop(numBins) { binIndex in
                let binIndexFloat = b.cast(binIndex, to: .float)
                ...
                b.loop(windowSize) { m in
                    let mFloat = b.cast(m, to: .float)
                    let sampleIdx = i - (winSize - b.constant(1.0)) + mFloat

                    let s1 = b.tapeLoad(sig1, at: sampleIdx)
                    let s2 = b.tapeLoad(sig2, at: sampleIdx)

                    let angle = b.constant(-2.0) * b.pi * binIndexFloat * mFloat / winSize
                    let c = b.cos(angle)
                    let s = b.sin(angle)
                ...
                // Sample gradient for position n
                ...
                let binContrib2 = (b.constant(0.0) - lossGrad) * sampleGrad2 * upstreamGrad
            ...
            let offset1 = i * winSize * b.constant(2.0) + nFloat * b.constant(2.0)
            let offset2 = offset1 + b.constant(1.0)

            _ = b.memoryWrite(scratchCell, b.cast(offset1, to: .int), totalContrib1.value)
            _ = b.memoryWrite(scratchCell, b.cast(offset2, to: .int), totalContrib2.value)
        }
    }
}
