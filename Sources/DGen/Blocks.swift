...
private func wrapWithBPTTLoops(
  ...
  let carryCellWriteSet = Set(carryCellWriteNodes.compactMap { g.nodes[$0] }.compactMap { node -> CellID? in
  ...
  for writeNodeId in carryCellWriteNodes {
    ...
    if case .memoryWrite(let cell) = writeNode.op {
      ...
      if let gradLz = ctx.values[gradInput] {
        ...
        result.append(UOp(op: .memoryWrite(cell, zeroConst, remappedGradLz), value: writeVar, kind: .scalar))
      }
    }
  }
