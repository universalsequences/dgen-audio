...
extension UOp {
    public func prettyDescription() -> String {
        ...
        case let .maxAxisMarker(nodeId, axis, inShape, outShape, inFrameAware, outFrameAware):
            opStr = "\(ANSI.cyan)maxAxisMarker\(ANSI.reset)(node=\(nodeId), axis=\(axis), in=\(inShape), out=\(outShape), inFA=\(inFrameAware), outFA=\(outFrameAware))"
        case let .meanAxisMarker(nodeId, axis, inShape, outShape, inFrameAware, outFrameAware):
