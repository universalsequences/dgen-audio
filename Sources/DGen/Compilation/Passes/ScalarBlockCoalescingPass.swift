  ...
  static func run(
    ...
    for (index, block) in blocks.enumerated() {
      ...
      let nextIsSequential =
        index + 1 < blocks.count && blocks[index + 1].sequentialFrameGroup == nil && plain[index + 1] && blocks[index + 1].executionDemand == block.executionDemand && blocks[index + 1].frameOrder == .sequential
