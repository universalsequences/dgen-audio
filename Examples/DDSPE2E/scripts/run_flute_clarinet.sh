#!/usr/bin/env bash
# Reference-conditioned timbre proof. Labels select same-instrument reference
# pairs in the loader, but are never supplied to the model.
set -euo pipefail
BIN="${BIN:-./.build/debug/DDSPE2E}"
CACHE="${CACHE:-.ddsp_cache_flute_clarinet}"
RUN_PREFIX="${RUN_PREFIX:-flute_clarinet}"
STEPS_A="${STEPS_A:-10000}"
STEPS_B="${STEPS_B:-2500}"
STEPS_C="${STEPS_C:-1500}"

[[ -f "$CACHE/manifest.json" ]] || { echo "Run prepare_flute_clarinet.sh first" >&2; exit 1; }
[[ -x "$BIN" ]] || swift build -c debug --product DDSPE2E

common=(
  --cache "$CACHE" --mode m2 --split train --instruments 2
  --reference-conditioning true --reference-features 64 --reference-latent 32
  --reference-classification-weight 1.0
