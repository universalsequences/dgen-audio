...
enum DatasetPreprocessor {
  static func preprocess(
    ...
    for (fileIndex, fileURL) in wavFiles.enumerated() {
      ...
      var samples = resampleLinear(samples: rawSamples, sourceRate: sourceRate, targetRate: config.sampleRate)
      ...
      for start in starts {
        ...
        let features = FeatureExtractor.extract(samples: chunk, sampleRate: config.sampleRate, config: config)
        ...
        entries.append(
          CachedChunkEntry(
            ...
            relativeChunkPath: "chunks/\(chunkFileName)"
          )
        )
    ...
    try writeJSON(manifest, to: cacheRoot.appendingPathComponent("manifest.json"))
  ...
  private static func chunkStartIndices(sampleCount: Int, chunkSize: Int, chunkHop: Int) -> [Int] {
    ...
    if sampleCount <= chunkSize { return [0] }
