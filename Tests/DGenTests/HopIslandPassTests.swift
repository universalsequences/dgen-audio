...
final class HopIslandPassTests: XCTestCase {
  ...
  func testIndependentStaticSetupPrecedesIslandWithoutSplittingItsLifetime() {
    let blocks = [
      block(.hopBased(hopSize: 8, counterNode: 100)),
      block(.static_),
      block(.hopBased(hopSize: 8, counterNode: 200)),
    ...
    XCTAssertEqual(
      ...
      [
        .block(1),
        .hopIsland(HopIsland(domain: HopDomain(hopSize: 8), blockIndices: [0, 2, 3])),
      ]
    )
  }

  func testConflictingStaticBlockStillSplitsIsland() {
    let produced = Lazy.variable(10, nil)
    let blocks = [
      block(.hopBased(hopSize: 8, counterNode: 100),
            ops: [UOp(op: .identity(.constant(0, 1.0)), value: produced)]),
      block(.static_, ops: [UOp(op: .identity(produced), value: .variable(11, nil))]),
      block(.hopBased(hopSize: 8, counterNode: 200)),
    ]
    XCTAssertEqual(HopIslandPass.buildRegions(for: blocks), [.block(0), .block(1), .block(2)])
  }

  func testStaticWriteCannotMoveBeforeAnEarlierWriteToTheSameCell() {
    let blocks = [
      block(.hopBased(hopSize: 8, counterNode: 100),
            ops: [UOp(op: .store(30, .constant(0, 1)), value: .variable(10, nil))]),
      block(.static_,
            ops: [UOp(op: .store(30, .constant(1, 2)), value: .variable(11, nil))]),
      block(.hopBased(hopSize: 8, counterNode: 200)),
    ]
    XCTAssertEqual(HopIslandPass.buildRegions(for: blocks), [.block(0), .block(1), .block(2)])
  }
