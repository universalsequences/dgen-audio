...
final class SpectralLossTests: XCTestCase {
        ...
        func testSpectralLossForwardPass() throws {
                ...
                print(
                        "   ✅ Test passed: spectral loss computed = \(String(format: "%.6f", stableLoss))"
                )
        }
        ...
        func testSpectralLossSmallDifference() throws {
                ...
                for i in [0, 10, 20, 40, 63, 64, 65, 100, frameCount - 1] {
                        if i < frameCount {
                                print(
                                        "      Frame \(i): loss = \(String(format: "%.6f", outputBuffer[i]))"
                                )
                        }
                }
                ...
                XCTAssertGreaterThan(
                        finalLoss, 0.0001, "Loss should be measurable for 10 Hz difference")
                XCTAssertLessThan(finalLoss, 5.0, "Loss should be small for 10 Hz difference")

                print(
                        "   ✅ Test passed: 10 Hz difference gives loss = \(String(format: "%.6f", finalLoss))"
                )
        }
        ...
        func testSpectralLossSameSignal() throws {
                ...
                XCTAssertLessThan(
                        finalLoss, 0.2, "Spectral loss should be low for same frequency signals")

                print(
                        "   ✅ Test passed: same frequency signals have loss = \(String(format: "%.6f", finalLoss))"
                )
        }
        ...
        func testSpectralLossSelfComparison() throws {
                ...
                let loss = g.spectralLoss(phase, phase, windowSize: windowSize)
                _ = g.n(.output(0), loss)
                ...
                print(
                        "   ✅ Test passed: self-comparison loss = \(String(format: "%.8f", finalLoss))"
                )
        }

        /// Diagnostic test: Check if 440 vs 2440 Hz produces non-zero loss
        func testDebugLargeDifference() throws {
                ...
                for i in [0, 10, 32, 63, 64, 65, 100, frameCount - 1] {
                        if i < frameCount {
                                print(
                                        "      Frame \(i): loss = \(String(format: "%.8f", outputBuffer[i]))"
                                )
                        }
                }
