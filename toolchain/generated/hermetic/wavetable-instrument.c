...
static float t7_g[VOICE_COUNT * SCRATCH_STRIDE] __attribute__((aligned(64))) = {0};
static float t27_g[VOICE_COUNT * SCRATCH_STRIDE] __attribute__((aligned(64))) = {0};
...
void dgen_process_v1(const float * const *in, float * const *out, uint32_t nframes, void *state, const DGenProcessContextV1 *context, const DGenHostServicesV1 *host) {
  ...
  for (int i = 0; i < frameCount; i += 1) {
    ...
    float t49 = t45 + t43;
    float t50 = (int)t46;
    float t51 = memory[0 + (isfinite(t50) ? (int) t50 : 0)];
    float t52 = (int)t47;
    float t53 = memory[0 + (isfinite(t52) ? (int) t52 : 0)];
    float t54 = (int)t48;
    float t55 = memory[0 + (isfinite(t54) ? (int) t54 : 0)];
    float t56 = (int)t49;
    float t57 = memory[0 + (isfinite(t56) ? (int) t56 : 0)];
    float t58 = 1.0 - t39;
    float t59 = t51 * t58;
    float t60 = t53 * t39;
    float t61 = t59 + t60;
